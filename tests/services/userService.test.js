...
const { getAllUsers, getUserById, createUser } = await import('../../services/userService.js');
...
test('getUserById should fetch user', async () => {
    const users = [{ id: 1, name: 'Adam Millner', email: 'adam.millner@gmail.com' }]
    query.mockResolvedValue({ rows: users })
...
test('createUser should return the created user when they are successfully created', async () => {
    const newUser = { id: 1, name: "Adam Millner", email: "adam.millner@gmail.com" }

    query.mockResolvedValue({ rows: [newUser] })

    const createUserResult = await createUser({ name: newUser.name, email: newUser.email });
